...
def get_std(q = None, EPOCH = None, epsilon = None, delta = None, verbose = True):
    ...
    if verbose:
        print(f'\n==> numerical 1:{round(std_analyszer_1, 4)}, RDP 2:{round(std_analyszer_2,4)}')
...
def get_decaying_noise_multiplyer(q = None, EPOCH = None, epsilon = None, delta = None, verbose = True):
    ...
    #                                                     noise_multiplier = the_std * ratio - i*(the_std * ratio - low_std)/ (EPOCH-1), 
    #                                                     sampling_probability = q) )
    #     max_self_compositions.append( int(1/q) )
    ...
    # eps_low, eps_est, eps_up = accountant.compute_epsilon(delta = 1e-6, num_self_compositions = max_self_compositions)
    # print(f'==> eps_low:{eps_low}, eps_est:{eps_est}, eps_up:{eps_up}')
    ...
    #     noise_multiplier=1.0977,
    #     sampling_probability=1/50,
    #     delta=1e-5,
    #     eps_error=0.1,
    #     delta_error=1e-10,
    #     max_compositions=100*50
    # )

    # eps_low, eps_estimate, eps_upper = accountant.compute_epsilon(num_compositions=100*50)
    # print(f'==> eps_low:{eps_low}, eps_est:{eps_estimate}, eps_up:{eps_upper}')


if __name__ == "__main__":
    # a = get_std(q = 100 / 50000, EPOCH = 100, epsilon = 8, delta = 1e-5, verbose = True)
    # print(a)
    get_decaying_noise_multiplyer(q = 1000 / 50000, EPOCH = 100, epsilon = 8, delta = 1e-5, verbose = True)

    # print(__decay_func(0 ,100, 2, 1))
