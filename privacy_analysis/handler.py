...
class MyBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            ''' original sampling [1,2,3] [4,5,6] [7,8,9]...'''
            # to_be_returned = self.index_choices[:self.expected_batch_size]
            # self.index_choices = self.index_choices[self.expected_batch_size:]
            # ## print('this batch size is', len(to_be_returned), id(self))
            ...
            # ''' iteration independent [1,2,3] [1,4,5] [7,8]...'''
            # self.indexes.append( np.random.choice(self.index_choices, number, replace = False) )
