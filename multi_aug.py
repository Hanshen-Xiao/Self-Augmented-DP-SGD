...
def multi_sample_svhn(dataset, index, times):
    inputs= []
    img = dataset.data[index]
    for i in range(times):
        inputs += [dataset.transform(Image.fromarray(np.transpose(g, (1, 2, 0)))) for g in img]
    targets = [dataset.labels[idx] for idx in index] * times
    return torch.stack(inputs), torch.tensor(targets)
